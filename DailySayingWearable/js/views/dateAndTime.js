...
define({
    ...
    def: function dateAndTimeDefine(req) {
        ...
        function updateTime() {
           ...
           var hour = dateHelper.format(setTimeView.getTime(),'HH');
           var min = dateHelper.format(setTimeView.getTime(),'MM');
           var day = dateHelper.format(setTimeView.getTime(),'dd');
        	var now = tizen.time.getCurrentDateTime();
        	
        	var date = new Date(now.getFullYear(), now.getMonth(), day, hour,min);
